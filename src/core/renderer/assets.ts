 ...
 * Legacy/source reference only — runtime piece rendering uses cburnettPieceUrl() instead.
 *
...
/**
 * Resolve the per-piece Cburnett SVG asset URL for a given color and role.
 * File naming: {w|b}{k|q|r|b|n|p}.svg  (e.g. wp.svg, bk.svg)
 *
 * Package layout (published):
 * - dist/index.js
 * - assets/pieces/cburnett/{piece}.svg
 *
 * Each URL uses a static string literal so bundlers (Vite/Webpack) can resolve
 * the asset reference at build time. Dynamic template literals are not used.
 */
