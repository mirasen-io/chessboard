...
- Phase 3.10 was completed.
- The animation architecture is now split so runtime/core owns animation planning and `Animator` owns RAF/session lifecycle, while `SvgRenderer` owns scene graph rendering only.
- Castling now uses the shared committed-animation pipeline and animates king + rook together.
- A suppressed-piece cleanup bug was fixed.
- Tests are green and manual runtime verification passed.
- Focus is shifting into Phase 4.1.
- Current likely next task: define the minimal extension slot model and DOM/SVG ownership contract before implementation.
- Assume no new durable project-specific AGENTS rule emerged beyond what is already captured, unless the review concludes otherwise.
- Assume no new project-agnostic AGENTS-template rule emerged beyond what is already captured, unless the review concludes otherwise.
